...
/*
 * X420 flow intent is carried in skb->mark.  The magic occupies the upper
 * sixteen bits so deployments can preserve unrelated low-level routing marks.
 * Classes: 1 handshake, 2 interactive, 3 steady, 4 bulk, 5 idle-resume.
 */
#define X420_MARK_MAGIC	0x04200000U
#define X420_MARK_MASK	0xffff0000U
#define X420_CLASS_MASK	0x000000ffU

static u8 fq_x420_band(const struct fq_sched_data *q,
		       const struct sk_buff *skb)
{
	u32 mark = READ_ONCE(skb->mark);
	u8 flow_class;

	if ((mark & X420_MARK_MASK) != X420_MARK_MAGIC)
		return fq_prio2band(q->prio2band,
				    skb->priority & TC_PRIO_MAX);

	flow_class = mark & X420_CLASS_MASK;
	switch (flow_class) {
	case 1: /* REALITY handshake */
	case 2: /* latency-sensitive interaction */
		return 0;
	case 4: /* sustained bulk transfer */
		return 2;
	case 3: /* steady proxy traffic */
	case 5: /* controlled idle restart */
	default:
		return 1;
	}
}
		     ...
		     struct netlink_ext_ack *extack)
{
	...
	if (tb[TCA_FQ_FLOW_DEFAULT_RATE])
		pr_warn_ratelimited("sch_x420q: defrate %u ignored.\n",
		   ...
		   struct netlink_ext_ack *extack)
{
	...
	q->band_flows[0].quantum = 12 << 16;
	q->band_flows[1].quantum = 4 << 16;
	...
	/* Mark persistently late ECN-capable packets after 5 ms of pacing lag. */
	q->ce_threshold		= 5ULL * NSEC_PER_MSEC;
...
MODULE_ALIAS_NET_SCH("x420q");

static int __init fq_module_init(void)
{
	int ret;

	fq_flow_cachep = kmem_cache_create("x420q_flow_cache",
...
MODULE_LICENSE("GPL");
MODULE_DESCRIPTION("X420 mark-aware Fair Queue Packet Scheduler");
