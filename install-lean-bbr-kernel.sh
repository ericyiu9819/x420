...
KERNEL_VERSION="6.12.93-bbrv1-kvm-netopt-ext4"
PKG_VERSION="6.12.93.bbrv1.2"
...
WORKDIR="${WORKDIR:-/root/x420-kernel-install}"
...
install_kernel() {
  ...
  fetch "$PKG_BASE/SHA256SUMS" "SHA256SUMS" || true

  if [ -s SHA256SUMS ]; then
    echo "== verify sha256 =="
    sha256sum -c SHA256SUMS
  else
    echo "warning: SHA256SUMS not found; continuing without checksum verification" >&2
  fi

  echo "== install packages =="
  dpkg -i "$IMAGE" "$HEADERS"
...
Recommended test reboot path:
  ...
  1. Confirm provider console/GRUB access is available.
  2. Reboot manually.
  3. Verify:

     uname -r
     sysctl net.ipv4.tcp_congestion_control net.core.default_qdisc
     modinfo tcp_bbr || true
     ip addr
     mount | grep ' / '
...
Rollback:
  ...
  2. Remove /etc/sysctl.d/99-lean-bbr-assist.conf and run sysctl --system.
